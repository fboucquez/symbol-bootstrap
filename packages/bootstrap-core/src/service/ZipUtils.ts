...
export class ZipUtils {
    ...
    public unzip(zipFile: string, innerFolder: string, targetFolder: string): Promise<void> {
        const zip = new StreamZip({
            file: zipFile,
            storeEntries: true,
        });
        this.logger.info(`Unzipping Backup Sync's '${innerFolder}' into '${targetFolder}'. This could take a while!`);
        let totalFiles = 0;
        let process = 0;
