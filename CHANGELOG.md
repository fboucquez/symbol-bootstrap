...
- Allowed `repeat` in nemesis's mosaic list. Harvest currency can be removed with `repeat:0`.
- Removed preset `light`. Not it's an assembly for the bootstrap preset `symbol-bootstrap -p bootstrap -a light`.
